...
DEFAULT_DRIVE_PATH = ["Projects", "JobBots", "jobbot-nrw"]


def main() -> None:
    parser = argparse.ArgumentParser(description="Upload a JobBot NRW run folder to Google Drive")
    parser.add_argument("--run-dir", required=True, type=Path, help="Folder containing email-report.txt and vacancies.json")
    parser.add_argument("--run-name", help="Drive run folder name. Defaults to the local run folder name.")
